...
def test_server_methods_have_signatures(calculator_stub_lines):
    ...
    # Value.Server should have read method
    assert "def read(self, _context, **kwargs) -> Awaitable[float]:" in content

    # Calculator.Server should have evaluate method
    assert "def evaluate(" in content
    assert "expression: Calculator.Expression | dict[str, Any]" in content
...
def test_server_methods_accept_context(calculator_stub_lines):
    """Server methods should accept _context parameter."""
    lines = calculator_stub_lines
    content = "".join(lines)

    # All server methods should have _context parameter
    assert "_context" in content
    # And **kwargs for additional parameters
...
def test_server_method_parameters_match_protocol(calculator_stub_lines):
    ...
    # Find Function.Server's call method - should have same params plus _context
    server_call_found = False
    for line in lines:
        if "def call(self, params: Sequence[float], _context," in line:
            server_call_found = True
            break

    assert server_call_found, "Server call method should have same params as Protocol"
