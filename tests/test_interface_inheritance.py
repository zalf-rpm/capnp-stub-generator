...
def test_simple_interface_inheritance(generated_dir):
    ...
    assert "class ClimateInstance(Identifiable, Protocol):" in content, (
        "ClimateInstance should extend Identifiable"
    )
    ...
    assert found_server_inheritance, (
        "ClimateInstance.Server should extend Identifiable.Server"
    )
...
def test_interface_with_persistent_inheritance(generated_dir):
    ...
    assert found_server_inheritance, (
        "Service.Server should extend both Identifiable.Server and Persistent.Server"
    )
...
def test_interface_inheritance_in_nested_interfaces(generated_dir):
    ...
    assert "class AdminMaster(Identifiable, Protocol):" in content, (
        "AdminMaster should extend Identifiable"
    )
    assert "class UserMaster(Identifiable, Protocol):" in content, (
        "UserMaster should extend Identifiable"
    )
    assert "class Runtime(Identifiable, Protocol):" in content, (
        "Runtime should extend Identifiable"
    )
...
def test_interface_method_inheritance_visibility(generated_dir):
    ...
    assert "class ClimateInstance(Identifiable, Protocol):" in model_stub, (
        "ClimateInstance should extend Identifiable"
    )
