...
class TestInterfaceGenerationContext:
    ...
    def test_create_factory_method(self):
        ...
        assert context.schema == mock_schema
        assert context.name == "Calculator"
...
class TestMethodSignatureCollection:
    ...
    def test_initialization(self):
        ...
        assert collection.server_method_signature == ""
        assert collection.uses_direct_struct_return is False
        assert collection.namedtuple_info is None
    ...
    def test_set_result_class(self):
        """Test setting Result Protocol class lines."""
        collection = MethodSignatureCollection("evaluate")
        lines = [
            "class EvaluateResult(Awaitable[EvaluateResult], Protocol):",
            "    value: int",
        ]

        collection.set_result_class(lines)

        assert collection.result_class_lines == lines
    ...
    def test_set_server_method(self):
        """Test setting server method signature."""
        collection = MethodSignatureCollection("handle")
        signature = "    def handle(self, context: Any, input: str) -> None: ..."

        collection.set_server_method(signature)

        assert collection.server_method_signature == signature

    def test_set_namedtuple_info(self):
        """Test setting NamedTuple info."""
        collection = MethodSignatureCollection("getResult")

        collection.set_namedtuple_info("GetResultResult", "value", "MyStruct")

        assert collection.uses_direct_struct_return is True
        assert collection.namedtuple_info == ("GetResultResult", "value", "MyStruct")
    ...
    def test_repr(self):
        ...
        collection.set_result_class(["line4"])
        ...
        assert "result_lines=1" in repr_str
    ...
    def test_complete_workflow(self):
        ...
        collection.set_result_class([])  # Direct return, no Result Protocol
        collection.set_request_helper(["def calculate_request(...) -> CalculateRequest: ..."])
        collection.set_server_method("    def calculate(self, context: Any) -> int: ...")
...
class TestInterfaceDTOIntegration:
    ...
    def test_full_interface_workflow(self):
        ...
        method_collection.set_server_method(f"    def add(self, context: Any, {param.to_server_param()}) -> int: ...")
    ...
    def test_multiple_methods_workflow(self):
        ...
        for method_name in ["add", "subtract", "multiply"]:
            ...
            method_collection.set_server_method(f"    def {method_name}(self, context: Any) -> int: ...")
            server_collection.add_server_method(method_collection.server_method_signature)
