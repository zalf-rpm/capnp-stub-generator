    ...
    def test_base_class_returns_base_type(self, dummy_stub_file):
        ...
        for i, line in enumerate(lines):
            ...
            if in_base_class and "def structField(self) ->" in line:
                ...
                assert "TestAllTypesBuilder" not in line, \
                    "Base class should NOT include Builder in return type"
                assert "TestAllTypesReader" not in line, \
                    "Base class should NOT include Reader in return type"
    ...
    def test_reader_class_returns_reader_type(self, dummy_stub_file):
        ...
        for i, line in enumerate(lines):
            ...
            if in_reader_class and "def structField(self) ->" in line:
                ...
                assert "TestAllTypesReader" in line, \
                    "Reader class should return Reader type"
                assert "TestAllTypesBuilder" not in line, \
                    "Reader should not return Builder type"
                found_struct_field = True
                break
    ...
    def test_builder_class_returns_builder_type(self, dummy_stub_file):
        ...
        for i, line in enumerate(lines):
            ...
            if in_builder_class and "def structField(self) ->" in line:
                ...
                assert "TestAllTypesBuilder" in line, \
                    "Builder class getter should return Builder type"
                assert "TestAllTypesReader" not in line, \
                    "Builder getter should not return Reader type"
    ...
    def test_list_fields_follow_same_pattern(self, dummy_stub_file):
        ...
        for line in lines:
            ...
            if in_base and "def structList(self) ->" in line:
                ...
                assert "Sequence[TestAllTypes]" in line, \
                    "Base class list should be Sequence[BaseType]"
        ...
        for line in lines:
            ...
            if in_reader and "def structList(self) ->" in line:
                assert "Sequence[TestAllTypesReader]" in line, \
                    "Reader class list should be Sequence[ReaderType]"
...
class TestInterfaceReturnTypes:
    ...
    def test_interface_has_no_builder_reader(self, interface_stub_file):
        ...
        assert "class Greeter(Protocol):" in content, \
            "Should have Protocol definition"
        
        # Should NOT have Builder/Reader variants
        assert "class GreeterBuilder" not in content, \
            "Interfaces should not have Builder class"
        assert "class GreeterReader" not in content, \
            "Interfaces should not have Reader class"
    ...
    def test_interface_methods_return_interface_type(self, interface_stub_file):
        ...
        assert "GreeterBuilder" not in lines_after_greeter, \
            "Interface methods should not reference Builder"
        assert "GreeterReader" not in lines_after_greeter, \
            "Interface methods should not reference Reader"
...
class TestStaticMethodReturnTypes:
    ...
    def test_new_message_returns_builder(self, dummy_stub_file):
        ...
        assert ") -> TestAllTypesBuilder:" in content, \
            "new_message should return Builder type"
    ...
    def test_from_bytes_returns_reader(self, dummy_stub_file):
        ...
        assert "-> Iterator[TestAllTypesReader]:" in content, \
            "from_bytes should return Reader type in Iterator"

    def test_read_returns_reader(self, dummy_stub_file):
        """read methods should return Reader type (read-only)."""
        content = dummy_stub_file.read_text()
        
        assert ") -> TestAllTypesReader:" in content, \
            "read should return Reader type"
    ...
    def test_reader_does_not_have_new_message(self, dummy_stub_file):
        ...
        assert "def new_message(" not in reader_text, \
            "Reader class should not declare new_message"
